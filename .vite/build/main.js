...
const dbPath = path.join(electron.app.getPath("userData"), "second-brain.db");
let db;
...
function getFeedById(id) {
  return db.prepare("SELECT * FROM feeds WHERE id = ?").get(id) ?? null;
}
function addFeed(url, title, description) {
  const result = db.prepare("INSERT INTO feeds (url, title, description) VALUES (?, ?, ?)").run(url, title, description);
  return db.prepare("SELECT * FROM feeds WHERE id = ?").get(result.lastInsertRowid);
}
function deleteFeed(id) {
  db.prepare("DELETE FROM feeds WHERE id = ?").run(id);
}
function updateLastFetched(id) {
  db.prepare("UPDATE feeds SET lastFetchedAt = datetime('now') WHERE id = ?").run(id);
}
function getFeedArticleCount(feedId) {
  const row = db.prepare("SELECT COUNT(*) as count FROM articles WHERE feedId = ?").get(feedId);
  return (row == null ? void 0 : row.count) ?? 0;
}
function getArticlesByFeed(feedId) {
  return db.prepare(
...
function upsertArticles(feedId, articles) {
...
function addTask(title, categoryId, priority) {
...
async function fetchAndParseFeed(url) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Server returned ${response.status} ${response.statusText || ""} for ${normalizedUrl}.${serverDetail} ` + (response.status === 404 ? "The feed URL was not found." : response.status >= 500 ? "The feed server encountered an internal error." : "Please verify the URL is correct.")
    );
  }
  ...
  if ((_a = parsed.rss) == null ? void 0 : _a.channel) {
  ...
  if (parsed.feed) {
    ...
    return {
      ...
      articles: entries.map((entry) => {
        var _a2, _b, _c;
        const link = Array.isArray(entry.link) ? ((_a2 = entry.link.find((l) => l["@_rel"] === "alternate")) == null ? void 0 : _a2["@_href"]) || ((_b = entry.link[0]) == null ? void 0 : _b["@_href"]) : ((_c = entry.link) == null ? void 0 : _c["@_href"]) || "";
        return {
...
function registerIpcHandlers() {
  electron.ipcMain.handle("feeds:getAll", () => getAllFeeds());
  electron.ipcMain.handle("feeds:getById", (_, id) => getFeedById(id));
  electron.ipcMain.handle("feeds:add", (_, url, title, description) => addFeed(url, title, description));
  electron.ipcMain.handle("feeds:delete", (_, id) => deleteFeed(id));
  electron.ipcMain.handle("feeds:updateLastFetched", (_, id) => updateLastFetched(id));
  electron.ipcMain.handle("feeds:getArticleCount", (_, feedId) => getFeedArticleCount(feedId));
  electron.ipcMain.handle("articles:getByFeed", (_, feedId) => getArticlesByFeed(feedId));
  electron.ipcMain.handle("articles:upsert", (_, feedId, articles) => upsertArticles(feedId, articles));
  electron.ipcMain.handle("articles:markAsRead", (_, id) => markAsRead(id));
  electron.ipcMain.handle("articles:toggleSave", (_, id) => toggleSaveArticle(id));
  electron.ipcMain.handle("articles:getSaved", () => getSavedArticles());
  electron.ipcMain.handle("tasks:getAll", () => getAllTasks());
  electron.ipcMain.handle("tasks:add", (_, title, categoryId, priority) => addTask(title, categoryId, priority));
  electron.ipcMain.handle("tasks:toggle", (_, id) => toggleTask(id));
  electron.ipcMain.handle("tasks:delete", (_, id) => deleteTask(id));
  electron.ipcMain.handle("categories:getAll", () => getAllCategories());
  electron.ipcMain.handle("categories:add", (_, name, color) => addCategory(name, color));
  ...
  electron.ipcMain.handle("shell:openExternal", (_, url) => {
    if (/^https?:\/\//i.test(url)) {
      return electron.shell.openExternal(url);
    }
  });
  electron.ipcMain.handle("rss:fetchAndParse", async (_, url) => {
    return fetchAndParseFeed(url);
  });
  electron.ipcMain.handle("rss:refresh", async (_, feedId) => {
